...
function App() {
  ...
  return <main>
    <aside><div className="brand"><span>◈</span> CENTRICA<br/><small>RETENTION INTELLIGENCE</small></div>
...
function AgentWorkspace({customers, openCustomer}) { const [id, setId] = useState("C-1002"); const current = customers.find(c => c.customer_id === id); return <div className="agent-grid"><section className="panel orchestration"><p className="eyebrow">NEURO SAN ORCHESTRATION</p><h2>Evidence-led assessment</h2><div className="flow"><article><span>01</span><b>Customer Risk Agent</b><small>Customer 360 + deterministic volatility score</small></article><article><span>02</span><b>Sentiment Agent</b><small>Feedback, NPS and CSAT interpretation</small></article><article><span>03</span><b>Intervention Agent</b><small>Policy-grounded human action recommendation</small></article></div></section><section className="panel prompt"><p className="eyebrow">DEMO PROMPT</p><h2>Assess a customer</h2><select value={id} onChange={e => setId(e.target.value)}>{customers.map(c => <option key={c.customer_id}>{c.customer_id}</option>)}</select><button className="assess" onClick={() => openCustomer(id)}>Run assessment →</button>{current && <div className="result"><span className={`pill ${tone(current.risk_band)}`}>{current.risk_band}</span><b>{current.volatility_score}/100 volatility</b><p>{current.intervention.recommended_action}</p><small>Chat endpoint becomes available when the local Neuro SAN server is running.</small></div>}</section></div>; }
